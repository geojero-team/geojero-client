...
/** 고현터미널 설명이 물러설 배율 — **축척 4km**, 9경 설명이 서는 배율과 같습니다(390 화면에서 축척바로 확인). */
const TERMINAL_FIT_LEVEL = 9
        ...
        <div className={styles.mapWrap} style={{ bottom: picked ? peekHeightOf(picked) : 0 }}>
          <MapView
            ...
            /* 고현터미널은 **한 곳**이라 그냥 맞추면 500m 까지 당겨져 거제 어디인지가 사라집니다.
               터미널을 가운데 두고 **9경 설명과 같은 축척 4km**로 물러섭니다(2026-09-22 사용자:
               「거제9경을 클릭했을 때처럼 고현터미널도 4km 확대 비율로 유지해 달라」) — 두 설명이 같은 배율이라야
               지도가 같은 지도로 읽히고, 「관광객들은 여기로만 온다」는 말도 주변이 보여야 읽힙니다.
               레벨 9 가 축척 4km 라는 것은 390 화면에서 축척바 글자를 읽어 확인했습니다(9경 설명도 이 배율에 섭니다).
               9경은 아홉 곳이 스스로 그 폭을 만들어 이 값이 필요 없습니다. */
            fitLevel={fitNine && tourTopic === 'terminal' ? TERMINAL_FIT_LEVEL : null}
            ...
            /* 말풍선을 몽꾸 왼쪽 가로로 옮기면서(2026-09-19) 아래를 덜 비워도 됩니다 —
               말풍선이 몽꾸 높이 안에 들어오므로 그 띠(≈140)만 피하면 됩니다.
               너무 올리면 축척이 4km 에서 8km 로 물러납니다(사용자가 4km 지정). */
            bottomReserved={fitNine ? 140 : 0}
