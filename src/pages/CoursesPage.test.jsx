...
describe('CoursesPage — v3 대표 코스 카드(585:417 · 585:485 · 582:416)', () => {
  it('머리 · 헤드라인 두 줄 · 출처 한 줄 · 「대표 코스 2가지」', async () => {
    ...
    expect(screen.getByText('교외에서 오면 고현터미널에서 출발하는 것이 매우 효율적이에요')).toBeInTheDocument()
...
describe('CoursesPage — 배지는 축마다 색 하나 · 숫자 없는 짧은 이름(코스재설계 §5-2 · §5-3 · 2026-09-17 저녁)', () => {
  ...
  it('NINE — 해 메달(장식) + 글자 「거제 9경」 · 번호 없음(읽기 도구도 「거제 9경」)', async () => {
    show(NINE)
    renderPage()
    ...
    // ◎ 도장을 뺐다가(2026-09-17 밤 사용자: "이게 좀 안 예뻐") 글자만으론 "아쉽다"고 해 시안 비교 뒤 1안 — 해 + 물결 메달.
    // 메달은 장식이라 읽기 도구에 숨긴다. 글자 없는 표식이라 숫자도 없다.
    expect(badgeOf(202).querySelector(`.${styles.badgeNineMark} svg`)).toHaveAttribute('aria-hidden', 'true')
