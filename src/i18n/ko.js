...
export default {
  ...
  // 뒤에 아무것도 없는 마지막 걸음(고현터미널 2단계) — 「다음」이라고 적으면 다음이 있는 줄 압니다.
  'nineScenic.tourDone': '알겠어요',
  ...
  /* ── 몽꾸에게 물어보는 것 둘(2026-09-22 사용자) ─────────────────────────────
     전에는 말풍선이 「거제 9경이 뭘까?」 하나뿐이었습니다. 고현터미널 이야기가 붙어 둘이 되면서
     말풍선이 **고르는 자리**가 되었으므로, 묶음에 읽기 도구용 이름을 답니다(버튼 둘이 무엇의 목록인지).
     문구는 둘 다 **질문**입니다 — 몽꾸가 묻고 사용자가 고릅니다. */
  'mascot.asks': '몽꾸에게 물어보기',
  'terminal.bubble': '고현터미널?',
  /* 고현터미널 설명 — 9경과 같은 모양으로 두 번에 나눠 말합니다(지도는 터미널 하나만 남기고 흐려집니다).
     「관광객들은 고현터미널로만 방문해요」는 사실이 아니라 **우리 가정**이라서, 두 번째 줄이 그 가정을 밝힙니다 —
     그래야 「왜 코스가 전부 고현터미널에서 시작하지?」에 답이 됩니다(화면 곳곳의 각주와 같은 말). */
  'terminal.lead1': '관광객들은 고현터미널로만 방문해요.',
  'terminal.lead2': '따라서 코스 추천도',
  'terminal.lead3': '고현터미널 시작을 가정한답니다!',
  ...
  /* ── 스팟 하트 (2026-09-21 사용자 결정) — Figma 에 없는 문구, 디자인브리프 부록 Q 가 그림 대신 ──
     상세에서 누르고 카드 · 줄에는 수만 보입니다. 누르는 것만 로그인, 보기 · 정렬은 비로그인.
     「추천순」= 하트 수 순(lib/listTools). 0 도 「♥ 0」으로 보입니다 — 숨기면 목록이 왜 이 순서인지 말하지 않습니다. */
  // 버튼 이름(aria-label) — 눌린 상태(aria-pressed)에 따라 둘 중 하나. 수는 설명(aria-describedby)으로 따로 들립니다.
  'spotLike.like': '하트 누르기',
  'spotLike.unlike': '하트 취소',
  // 카드 · 줄 · 버튼 안의 수 — 읽기 도구 이름. 아이콘은 읽히지 않으므로 「하트」를 글로 붙입니다.
  'spotLike.count': '하트 {n}',
  // 코스 저장 · 사진 올리기의 로그인 시트와 제목만 다릅니다.
  'spotLike.loginTitle': '하트를 누르려면 로그인 해주세요',
  // 0 과 실패는 다른 답입니다 — 실패는 버튼 아래 한 줄로 말하고 수는 그대로 둡니다(방문자 사진의 알림 줄과 같은 자리).
  'spotLike.failed': '하트를 누르지 못했어요. 잠시 뒤 다시 시도해 주세요',
  ...
  // 선착장은 출발점을 알 수 없어 길찾기를 줄 수 없습니다 — 그 자리를 지도에 띄웁니다(2026-09-21, boarding.view 와 한 짝).
  'boat.viewA11y': '{name} 선착장 위치 — 카카오맵에서 열려요',
  ...
  // 출발에서 정류장까지 도보 길찾기(2026-09-16 사용자 결정 — 그림은 「카카오맵으로 길찾기 ↗」).
  'boarding.walk': '카카오맵으로 도보 길찾기 ↗',
  'boarding.walkA11y': '{place}에서 {name} 정류장까지 카카오맵 도보 길찾기 — 새 창에서 열려요',
  /* 타는 곳이 출발지와 같은 자리면(고현터미널 앞 정류장) 길찾기가 성립하지 않아 **지도에 띄우기만** 합니다
     (2026-09-21 사용자 — 전에는 목적지만 넘겨서 출발지가 빈 **자동차** 길찾기가 열렸습니다).
     누르면 뭐가 열리는지 버튼이 말해야 하므로 글자도 「보기」입니다(2026-09-16 과 같은 원칙). 선착장(DockCard)도 이 글자를 씁니다. */
  'boarding.view': '카카오맵에서 보기 ↗',
  'boarding.viewA11y': '{name} 정류장 위치 — 카카오맵에서 열려요',
  ...
  // 한 줄에 들어가게 줄였습니다(2026-09-20) — 두 줄로 접히면 제안이 설명문처럼 읽힙니다.
  'courseQuiz.entryHint': '질문 세 개면 코스가 나와요',
