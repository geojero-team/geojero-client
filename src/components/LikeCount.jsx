 ...
 */
export default function LikeCount({ count, size = 12, filled = false, id, className }) {
  if (count == null) return null
  const label = t('spotLike.count', { n: count })
  return (
    <span id={id} className={className ? `${styles.count} ${className}` : styles.count} role="img" aria-label={label}>
